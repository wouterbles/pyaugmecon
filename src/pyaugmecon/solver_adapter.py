...
#   before each solve.
# * `_MANAGE_ENV_BACKENDS`: accept `manage_env=True` at factory construction
...
# * `_RELEASE_METHOD`: teardown method to call at end-of-life so Gurobi licenses
#   are not held across runs.
_PERSISTENT_BACKENDS = frozenset(
    {"gurobi_persistent", "cplex_persistent", "xpress_persistent"}
)
_MANAGE_ENV_BACKENDS = frozenset({"gurobi_direct", "gurobi_persistent"})
_RELEASE_METHOD = {
    "gurobi": "close",
    "gurobi_direct": "close",
    "gurobi_persistent": "close",
    "appsi_gurobi": "release_license",
}
...
def _solver_factory_kwargs(
    ...
    manage_env = candidate in _MANAGE_ENV_BACKENDS or (
        candidate == "gurobi" and config.solver_io in {"direct", "python"}
    )
    if manage_env:
...
def release_solver(solver: SolverLike, backend_name: str) -> None:
    """Release backend resources that otherwise keep Gurobi licenses active."""
    method_name = _RELEASE_METHOD.get(backend_name)
...
def select_solver(
    ...
    raise RuntimeError(
        ...
        "Install/configure one of the backends and verify license availability."
    )
...
def solve_once(
    ...
    Every supported backend accepts `(model, load_solutions=False, warmstart=...)`.
    APPSI and persistent backends materialize the solution via `solver.load_vars()`;
    classic backends rely on `model.solutions.load_from(result)`. We probe at
    runtime so test stubs and unusual third-party backends both work.
    """
    _prepare_persistent_backend(model, solver, backend_name)
    result = solver.solve(model, load_solutions=False, warmstart=warmstart)
