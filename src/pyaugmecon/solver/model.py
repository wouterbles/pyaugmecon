...
class Model:
    def __init__(self, model: pyo.ConcreteModel, config: PyAugmeconConfig):
        ...
        # Setup solves: `n_obj**2` for the lex payoff matrix, plus one
        # minimization per constrained objective when the safe strategy
        # auto-computes nadirs. Explicit nadirs and the payoff strategy
        # both reuse the payoff table and add no setup solves.
        constrained_count = len(self.constrained_objectives)
        needs_safe_nadir_solves = (
            config.nadir_points is None and config.nadir_strategy == "safe"
        )
        self.setup_solve_count = self.n_obj**2 + (
            constrained_count if needs_safe_nadir_solves else 0
        )

        # Best-effort progress-bar total. Refined in `find_obj_range` once
        # the actual grid sizes are known.
        points = config.get_points_per_objective(constrained_count)
        self.to_solve = self.setup_solve_count + (int(prod(points)) if points else 0)
    ...
    def _compute_payoff_nadirs(self) -> dict[int, float]:
        """Compute nadir lower bounds from the payoff table column minima.

        This is the classic AUGMECON / AUGMECON-R approximation: take the
        worst lex-payoff value seen for each objective (the column minimum
        of `self.payoff` after `min_to_max`) and widen it by `nadir_undercut`.

        Two branches handle sign flips: when the column minimum is negative,
        multiplying by `undercut < 1` would actually move the bound *up*, so
        we also try `1/undercut` and keep the smaller (looser) bound.

        This bound is not provably safe -- the column minimum is the
        anti-ideal point, which can be larger than the true nadir for >= 3
        objectives -- but it matches the literature and avoids the extra
        per-objective minimization solves the safe strategy needs.
        """
        undercut = self.config.nadir_undercut
        log.debug("Computing payoff nadir bounds (undercut=%s)", undercut)
        col_mins = np.min(self.payoff, axis=0)
        return {
            objective_idx: min(
                round(col_mins[objective_idx] * undercut, 0),
                round(col_mins[objective_idx] / undercut, 0),
            )
            for objective_idx in self.constrained_objectives
        }
    ...
    def find_obj_range(self) -> None:
        ...
        # workers decode via divmod (see `worker.py`).
        ...
        # `setup_solve_count` was finalized in `__init__`; refine the
        # progress-bar total with the now-known grid size.
    ...
    def _resolve_nadirs(self) -> dict[int, float]:
        """Pick explicit nadir points from config or auto-compute them.
        ...
        used internally. When auto-computing, dispatch on `nadir_strategy`:
        `"safe"` solves an extra single-objective minimization per constrained
        objective; `"payoff"` reuses the payoff column minima with an
        `nadir_undercut` widening factor.
        """
        if self.config.nadir_points is not None:
            return {
                objective_idx: float(self.config.nadir_points[pos])
                * self.obj_goal[objective_idx]
                for pos, objective_idx in enumerate(self.constrained_objectives)
            }
        if self.config.nadir_strategy == "payoff":
            return self._compute_payoff_nadirs()
