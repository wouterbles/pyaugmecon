...
def unit_commitment_model():
    ...
    def cost_objective(model):
        return sum(sum(sum(
            model.C[i, f]*model.b[i, f, t] for f in model.F)
            + model.CSU[i, t] + model.CSD[i, t] for i in model.I)
            for t in model.T)

    def emissions_objective(model):
        return sum(sum(
            model.P[i, t]*model.Emissions[i] for i in model.I)
            for t in model.T)
    ...
    def ramp_down_rule(model, i, t):
        ...
        if model.T.ord(t) > 1:
            return (model.P[i, model.T.prev(t)] - model.P[i, t]) \
                <= 60*model.RD[i]

    def start_up_cost(model, i, t):
        if model.T.ord(t) == 1:
            return model.CSU[i, t] \
                >= model.SUC[i]*(model.u[i, t] - model.uini[i])

        if model.T.ord(t) > 1:
            return model.CSU[i, t]  \
                >= model.SUC[i]*(model.u[i, t] - model.u[i, model.T.prev(t)])

    def shut_down_cost(model, i, t):
        if model.T.ord(t) == 1:
            return model.CSD[i, t] \
                >= model.SDC[i]*(model.uini[i] - model.u[i, t])

        if model.T.ord(t) > 1:
            return model.CSD[i, t] \
                >= model.SDC[i]*(model.u[i, model.T.prev(t)] - model.u[i, t])

    def ESS_SOEupdate(model, s, t):
        if model.T.ord(t) == 1:
            return model.SOE[s, t] \
                == model.ESS_SOEini[s] + model.ESS_Eff[s]*model.Pch[s, t] \
    ...
    def Balance(model, t):
        return \
            model.PV[t] \
            + sum(model.P[i, t] for i in model.I) \
            + sum(model.Pdis[s, t] for s in model.S) \
            == model.SystemDemand[t] - model.Pres[t] \
            + sum(model.Pch[s, t] for s in model.S)
