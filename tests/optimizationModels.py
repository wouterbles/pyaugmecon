...
from pyomo.core.base import (
    Var, ConcreteModel, Constraint, ObjectiveList, maximize, minimize,
    NonNegativeReals)


def twoObjectiveModel():
    model = ConcreteModel()
    ...
    model.objList.add(expr=objective1(model), sense=maximize)
    model.objList.add(expr=objective2(model), sense=maximize)
...
def threeObjectiveModel():
    ...
    model.objList.add(expr=objective3(model), sense=minimize)
