...
def test_constructor_accepts_custom_log_sink():
    capture = _CaptureLogSink()
    ...
    assert any("custom_log_sink_test" in message for message in capture.messages)


def test_solve_deactivates_all_objectives_automatically(monkeypatch):
    model = two_objective_model()
    ...
    monkeypatch.setattr(py_augmecon.model, "min_to_max", stop_after_check)
